...
type Replay = {
  match: { home_team: string; away_team: string; kickoff_at: string | null; finished_at: string | null };
...
function buildEvents(replay: NonNullable<Replay>) {
  ...
  if (replay.match.finished_at) events.push({ key: "final", at: replay.match.finished_at, time: formatTime(replay.match.finished_at), title: "Final score", body: `Match finished ${replay.resolutions[0]?.final_score ?? "with final score pending"}.`, icon: Trophy });
