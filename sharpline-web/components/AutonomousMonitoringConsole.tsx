...
export function AutonomousMonitoringConsole({ agentState, fixtures, recentSignals }: { agentState: AgentState | null; fixtures: Fixture[]; recentSignals: Signal[] }) {
  ...
  const statusItems = [
    { label: "Agent Active", value: statusValue(agentState?.worker_status), icon: Server },
    ...
    { label: "Live Monitoring", value: monitoredFixtures === null ? "Monitoring next World Cup kickoff" : `${monitoredFixtures} World Cup fixture${monitoredFixtures === 1 ? "" : "s"}`, icon: Bot },
    { label: "Agent Pulse", value: relativeTime(agentState?.last_heartbeat_at), icon: Clock },
    { label: "Last Stored Event", value: relativeTime(lastSupabaseWrite), icon: Database },
    { label: "Last AI Explanation", value: lastAiAnalysis ? absoluteTime(lastAiAnalysis) : "Waiting for first qualifying market movement", icon: Brain },
  ];
    ...
    <section className="premium-card overflow-hidden">
      <div className="border-b border-border p-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <h2 className="mt-3 font-display text-3xl font-semibold tracking-[-0.03em] text-text">Agent active and monitoring TxLINE</h2>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-text-muted">The autonomous agent listens to TxLINE, watches upcoming fixtures, stores verified events, and generates explanations only when real market movement qualifies.</p>
          </div>
      ...
      <div className="p-6">
        ...
        <div className="mt-5 space-y-3">
          {events.length ? events.map((event, index) => <ActivityRow key={`${event.label}-${event.at ?? index}`} event={event} />) : <div className="rounded-xl border border-border bg-bg/50 p-4 text-sm text-text-muted">Monitoring next kickoff. Activity appears here when TxLINE sends fixture, odds, or signal updates.</div>}
        </div>
      </div>
    </section>
  );
...
function statusValue(value: string | number | boolean | null | undefined) { return value === null || value === undefined || value === "" ? "Monitoring next kickoff" : title(String(value)); }
...
function relativeTime(value: string | null | undefined) { if (!value) return "Monitoring next kickoff"; const seconds = Math.max(0, Math.round((Date.now() - new Date(value).getTime()) / 1000)); if (seconds < 60) return `${seconds} seconds ago`; const minutes = Math.round(seconds / 60); if (minutes < 60) return `${minutes} minutes ago`; return `${Math.round(minutes / 60)} hours ago`; }
