...
export function AgentStatus({ agentState }: { agentState: AgentState | null }) {
  const checks = [
    `Worker: ${title(agentState?.worker_status) ?? "Not reported yet"}`,
    `TxLINE: ${title(agentState?.txline_status) ?? "Not reported yet"}`,
    `State: ${stateLabel(agentState?.current_state)}`,
    `Odds updates: ${agentState?.odds_updates_processed ?? "Not reported yet"}`,
    `Signals: ${agentState?.signals_generated ?? "Not reported yet"}`,
  ];
  return (
    <section className="rounded-2xl border border-border bg-surface p-6">
      ...
      <p className="mt-4 max-w-4xl text-sm leading-6 text-text-muted">{agentState?.notes ?? "Not reported yet"}</p>
