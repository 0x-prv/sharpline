        ...
        <div className="grid gap-6 lg:grid-cols-[1.05fr_.95fr] lg:items-end">
          ...
          <div className="premium-card p-4">
            ...
            <div className="mt-4 space-y-3">
              {signals.length ? signals.slice(0, 3).map((signal, index) => <LiveSignalCard key={`${signal.occurred_at}-${index}`} signal={signal} />) : <EmptyLiveSignal />}
            </div>
          </div>
        </div>
...
function EmptyLiveSignal() { return <div className="rounded-2xl border border-border bg-bg/50 p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-sm font-semibold text-text">No live AI signal yet</p><p className="mt-1 text-xs leading-5 text-text-muted">Autonomous monitoring is active. Real TxLINE signals will appear after qualifying score or odds movement.</p></div><span className="rounded-full border border-signal-green/20 bg-signal-green/10 px-2.5 py-1 font-data text-[11px] text-signal-green">Listening</span></div><div className="mt-4 grid grid-cols-4 gap-2"><Mini label="Conf" value="Waiting" /><Mini label="Momentum" value="Waiting" /><Mini label="Risk" value="Waiting" /><Mini label="Detected" value="Waiting" /></div></div>; }
