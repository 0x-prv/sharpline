          ...
          <div key={match.id} className="rounded-xl border border-border bg-bg/50 p-4">
            <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-text-muted">Final {match.final_score ?? "Score pending"} · {match.finished_at ? new Date(match.finished_at).toLocaleDateString("en-US", { month: "short", day: "numeric" }) : "Completed"}</p>
              </div>
