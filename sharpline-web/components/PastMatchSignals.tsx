...
export function PastMatchSignals({ matches }: { matches: CompletedMatch[] }) {
  return (
    <section className="rounded-2xl border border-border bg-surface p-6">
      ...
      <h2 className="mt-3 font-display text-2xl font-semibold text-text">Past match intelligence from verified production data.</h2>
      <p className="mt-2 text-sm leading-6 text-text-muted">Shows final scores, captured signals, replay availability, and outcome tracking only when TxLINE-backed rows exist.</p>
      <div className="mt-5 space-y-3">
        {matches.length === 0 ? (
          <div className="rounded-xl border border-border bg-bg/50 p-6 text-sm text-text-muted">No completed TxLINE-backed matches are available yet. Once TxLINE provides verified final scores and SharpLine has stored signals, past match intelligence will appear here automatically.</div>
        ) : matches.map((match) => (
          <div key={match.id} className="rounded-xl border border-border bg-bg/50 p-4">
            <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-text-muted">{match.final_score ? `Final ${match.final_score}` : "No verified final score yet"} · {match.status} · {match.finished_at ? new Date(match.finished_at).toLocaleDateString("en-US", { month: "short", day: "numeric" }) : "Completed"}</p>
                {match.signals === 0 ? <p className="mt-2 text-sm text-signal-amber">No live signal was generated for this match</p> : null}
            ...
            <div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-7">
              ...
              <Mini label="Accuracy" value={match.accuracy === null ? "Awaiting resolution" : `${match.accuracy}%`} />
              <Mini label="Best Signal" value={match.bestSignal ?? "No signal"} />
              <Mini label="Largest Move" value={match.largestOddsMovement === null ? "No movement stored" : `${match.largestOddsMovement.toFixed(1)}%`} />
...
function formatNumber(value: number | null | undefined) {
  return value === null || value === undefined ? "Awaiting result" : `${value > 0 ? "+" : ""}${value}`;
}
